  ...
  class BF_Admin_Page_Class {
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        <div class="options_page_title" style="float:left">'."\n";
      ...
      /*
<div class="options_page_save_btn1" style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" value="'.esc_attr(__('Save Changes','apc')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn button-primary">
        </div>'."\n".'
      <br style="clear:both">
*/
        ...
        if (method_exists($this,'show_field_' . $field['type'])){
          ...
          switch($field['type']) {
            ...
            case 'TABS_Listing':
              ...
              echo '</ul>'."\n".'</div>'."\n".'<div class="sections">'."\n".'<div class="options_page_save_btn1" style="float:right;">'."\n".'
          <input type="submit" value="'.esc_attr(__('Save Changes','apc')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn button-primary">
        </div>'."\n";
              break;
            case 'OpenTab':
              $this->tab_div = true;
    ...
    echo '</div>'."\n".'<div style="clear:both"></div>'."\n".'<div class="footer_wrap">'."\n".'
        <div class="options_page_save_btn2" style="float:right;">'."\n".'
          <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn button-primary" value="'.esc_attr(__('Save Changes','apc')).'" />'."\n".'
        </div>'."\n".'
        <br style="clear:both">
      </div>'."\n";
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_begin( $field, $meta) {
    ...
    // If desc_above_items is set and is true display the field's description
	if ( !empty($this->_Page_Config['desc_above_items']) ) {
	   	 $this->show_field_description( $field );
   	 }
  ...
  public function show_field_end( $field, $meta = NULL, $group = false) {
	...
	// If desc_above_items is not set or is false display the field's description
	if ( empty($this->_Page_Config['desc_above_items']) ) {
	   	 $this->show_field_description( $field );
   	 }
  ...
  public function show_field_text( $field, $meta ) {  
    $this->show_field_begin( $field, $meta );
    	echo "<input type='text' class='at-text".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}' id='{$field['id']}' value=\"".stripslashes(htmlentities(html_entity_decode($meta)))."\" size='30' style='".(isset($field['style'])? "{$field['style']}": "")."'/>"."\n";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_select( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      echo "<select class='".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}" . ((isset($field['multiple']) && $field['multiple']) ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . " style='".(isset($field['style'])? "{$field['style']}": "")."'>\n";
